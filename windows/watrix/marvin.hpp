...
/*
 * ----------------------------------------------------------------------------
 * watrix: A Minimalist GPU-only N-Dimensional ConvNets Framework
 ...
 */
    ...
    #define CPUStorage2ComputeT(x) (watrix::cpu_half2float(x))
    #define CPUCompute2StorageT(x) (watrix::cpu_float2half(x))
...
__global__ void Kernel_OpenCV_BGR_image_to_watrix(size_t CUDA_NUM_LOOPS, size_t N, size_t channels, size_t height, size_t width, const uint8_t* pIn, uint8_t* pOut) {
...
void OpenCV_BGR_image_to_watrix(size_t channels, size_t height, size_t width, const uint8_t* pIn, uint8_t* pOut){
    size_t N = channels * height * width;
    Kernel_OpenCV_BGR_image_to_watrix<<<CUDA_GET_BLOCKS(N), CUDA_NUM_THREADS >>>(CUDA_GET_LOOPS(N), N, channels, height, width, pIn, pOut);
}
...
public:
    ...
    size_t numel(){ return watrix::numel(dim); };

    size_t numBytes(){ return sizeof(T)*numel(); };

    int numofitems(){ return dim[0]; };

    size_t sizeofitem(){ return watrix::sizeofitem(dim); };
...
public:
    ...
    size_t numBytes(){ return sizeofStorageT*(watrix::numel(dim)); };
    ...
    int checkNaN(){
        return watrix::checkNaN(dataGPU, numel(dim));
    };

    int checkNaNdiff(){
        return watrix::checkNaN(diffGPU, numel(dim));
    };
...
public:
    ...
    int checkNaNWeight(){
        return watrix::checkNaN(weight_dataGPU, numel(weight_dim));
    };

    int checkNaNWeightDiff(){
        return watrix::checkNaN(weight_diffGPU, numel(weight_dim));
    };    
    ...
    int checkNaNBias(){
        return watrix::checkNaN(bias_dataGPU, numel(bias_dim));
    };

    int checkNaNBiasDiff(){
        return watrix::checkNaN(bias_diffGPU, numel(bias_dim));
    };       
    ...
    void train(int iter_begin = 0){
        ...
        for (iter=iter_begin;iter<=max_iter;++iter){
            ...
            if (iter!=iter_begin && iter % snapshot_iter==0){
                saveWeights(path+"_snapshot_"+std::to_string(iter)+".watrix",false);
            }
